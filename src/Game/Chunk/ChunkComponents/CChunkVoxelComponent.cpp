...
const SBlockInfo& CChunkVoxelComponent::getBlock(std::uint8_t x, std::uint8_t y, std::uint8_t z)
{
...
const SBlockInfo& CChunkVoxelComponent::getBlock_nothreadsafe(std::uint8_t x, std::uint8_t y, std::uint8_t z)
{
    if(m_blocks)
        return m_blocks[x + CHUNK_SIZE * (y + CHUNK_SIZE*z)];
    return m_majority;
...
void CChunkVoxelComponent::setBlock(std::uint8_t x, std::uint8_t y, std::uint8_t z, blocktype id)
{
    if (x == 0)
